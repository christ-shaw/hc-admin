...
/** Read the current products while keeping compatibility with legacy flat orders. */
...
/** Keep the outbound display format identical to generateOutboundFromOrders. */
...
/** Aggregate all linked order products into the pending outbound snapshot. */
...
function isOrderLinkedPendingOutbound(outbound) {
  return !!outbound
    && outbound.source === 'order'
    && outbound.outboundStatus === 'pending'
    && Array.isArray(outbound.orderIds)
    && outbound.orderIds.length > 0;
}
