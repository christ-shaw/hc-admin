...
在订单列表中，为订单状态为 `--` 的订单提供“申请快递”能力，并支持顺丰沙箱测试环境与生产环境动态切换。
...
- 沙箱和生产环境通过系统设置动态切换，不改业务代码、不重新部署。
...
`SF_ENV` 支持 `sandbox`、`sbox`、`production`、`prod`。它只作为 `system_config/sf_express.env` 缺失时的回退值。

### 3.3 环境动态切换（系统设置）

后台「系统设置」中提供“顺丰下单环境”选择器，保存后写入数据库文档 `system_config/sf_express` 的 `env` 字段（`sandbox` | `production`）。

五个 SF 相关云函数（`manageSfConfig`、`getSfAccessToken`、`applySfExpress`、`querySfOrderResult`、`cancelSfExpress`）共用同一份环境定义；其中下单、查询、取消、token 获取在每次调用时都会读取该文档，因此**切换后全链路即时生效，无需改环境变量、无需重新部署**。

```json
// system_config 集合，_id = "sf_express"
{ "env": "sandbox" }
```

规则：

- 文档不存在或 `env` 为空时，回退到各函数自己的 `SF_ENV` 环境变量（再缺省为 `sandbox`）
- `env` 值非法（不是 sandbox/production 及其别名）时函数直接报错，不会静默回退，避免误发环境
- 四个函数读同一条文档，天然保持一致；`getSfAccessToken` 与调用方之间的期望环境校验仍然保留
- 查询和取消会校验订单记录中的 `sfEnv`，如果订单申请环境与当前系统环境不一致，会提示先切换环境后再操作
- 沙箱/生产的客户编码、校验码等敏感配置仍只放云函数环境变量，数据库开关只决定「用哪套」
- 切到 production 前需确保：`SF_PROD_CLIENT_CODE`、`SF_PROD_CHECK_WORD`、月结卡号、寄件人等生产配置已在云函数环境变量中配置完成
- `sf_tokens/sandbox`、`sf_tokens/production` 会在首次获取 token 时自动创建，无需手工创建占位文档
...
`manageSfConfig` 用于设置页读写当前环境；其余四个函数负责顺丰 token、下单、查询、取消。
...
sequenceDiagram
  ...
  participant Token as getSfAccessToken
  participant DB as CloudBase DB
  participant SF as 顺丰 API

  FE->>CF: orderId
  CF->>DB: 读取 system_config/sf_express.env
  CF->>DB: 查询订单
  CF->>CF: 校验状态和收件信息
  CF->>CF: 生成 requestID 和 sfOrderId
  CF->>DB: 标记 expressApplyStatus=applying
  CF->>Token: 传入 sfEnv 获取或刷新 accessToken
  Token->>DB: 读取/写入 sf_tokens/{env}
...
- 如果上次 `failed`，允许重试
- 取消成功后再次申请会使用 `HC_${orderId}_2`、`HC_${orderId}_3` 等新客户订单号，因为顺丰取消后的客户订单号不可复用
- 遇到客户订单号重复时，系统会先调用顺丰查询接口尝试恢复已有运单；如果顺丰明确返回未查询到订单，会自动递增为下一个客户订单号继续重试，默认最多 5 次，可通过 `SF_DUPLICATE_ORDER_RETRY_LIMIT` 调整
...
- 在“全部设置”中展示当前顺丰环境
- 保存后调用 `manageSfConfig` 写入 `system_config/sf_express.env`
- 生产环境需要明确显示，避免误切
...
- 已生成顺丰客户订单号的订单可查询顺丰订单；已生成顺丰订单信息的订单可取消发货
...
- 重复下单：下单接口返回 `8016` 或重复订单提示时，自动调用顺丰订单查询接口恢复运单号；恢复失败才提示用户处理
