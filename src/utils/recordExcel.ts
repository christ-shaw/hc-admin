...
import { extractDateString } from './format';
...
export function exportOutboundRecordsExcel(records: OutboundRecord[], startDate: string, endDate: string) {
  ...
  const rows: Array<Array<string | number>> = [
    ['出库日期', '状态', '客户名称', '快递单号', '手机型号', '数量', '快递方式', '来源', '收货人', '收货电话', '收货地址', '是否异常', '备注'],
    ...dataRows,
  ];

  writeWorkbook(rows, [12, 10, 20, 22, 28, 10, 12, 12, 14, 16, 32, 10, 30], '出库记录', formatFilename('出库记录', startDate, endDate));
}
